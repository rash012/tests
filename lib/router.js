    ...
    data: function () {
        ...
        return {
            socialEvents: function () {
                ...
                if (types) {
                    return Events.find({
                        date: formatDate.toIso(params.year, params.month, params.day),
                        type: {$in: types}
                    }, {sort: {begin: 1}});
                }
                return Events.find({date: formatDate.toIso(params.year, params.month, params.day)}, {sort: {begin: 1}});
            },
...
Router.route('/my-event-orders', {
    ...
    data: function () {
        if (Meteor.userId()) {
            ...
            return {
                socialEvents: Events.find({_id: {$in: eventIds},'owner._id':{$ne:Meteor.userId()}}, {sort: {begin: 1}})
            }
        }
    }
});
