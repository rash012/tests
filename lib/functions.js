...
formatDate = {
    toIsoFrom3args: function (year, month, day) {
        var dd = day;
        if (dd < 10) dd = '0' + dd;
    ...
    toLocalFrom3args: function (year, month, day) {
        var dd = day;
        if (dd < 10) dd = '0' + dd;
    ...
    toLocalFromIsoString: function (dateString) {
...
getTypesFromQuery = function () {
    var typeParams = Router.current().params.query.type;
    if (typeParams)return typeParams.split(',');
    return false;
};

/**
 * Проверяет наступление дэдлайна по событию
 * @param {Date} date
 * @returns {boolean}
 */
wasHappenEventCreateDeadline = function (date) {
    return (date.getTime() - new Date().getTime()) < (1000 * 60 * 60 * eventCreateDeadline);
};
