...
Meteor.methods({
    insertEvent: function (eventObj) {
        ...
        if (wasHappenEventCreateDeadline(date)) return {wasHappenEventCreateDeadline: true};
    ...
    editEvent: function (eventId, eventObj) {
        ...
        //сохраним в объект изменения события
        var changesObj = {};
        _.each(eventObj, function (value, key, list) {
            if (prevEvent[key] !== value) changesObj[key] = value;
        });

        if (_.isEmpty(changesObj)) return {changes: false};

        if (!isOwner(eventId)) throw new Meteor.Error('Чужое событие');

        Events.update(eventId, {$set: changesObj}, function (error) {
            if (error) throw new Meteor.Error(error);
        });

        Meteor.call('eventNotification', eventId, notificationTypes.editEvent, changesObj);

        changesObj.eventId = eventId;
    ...
    shiftEvent: function (eventId, eventObj) {
        ...
        var date = new Date(eventObj.date + ' ' + eventObj.begin);
        if (wasHappenEventCreateDeadline(date)) {
            return {
                wasHappenEventCreateDeadline: true,
                eventId: eventId
            };
        }
